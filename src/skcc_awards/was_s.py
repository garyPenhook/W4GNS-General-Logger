"""
SKCC WAS-S Award - Worked All States (Senator)

The WAS-S Award recognizes contacts with Senator SKCC members in all 50 US states.
An operator must show contacts in which they have exchanged names and SKCC
numbers with Senator members in each of the 50 states.

Rules:
- Contact Senator (S) members in all 50 US states
- CRITICAL: Only Senator members count (NOT Tribune)
- CW mode exclusively
...
- WAS-S-QRP endorsement available (≤5W power)
- Mobile/portable Senator operations count regardless of home state
- Applicants need not hold Senator status themselves
"""
...
# WAS-S effective date
WASS_EFFECTIVE_DATE = "20160201"  # February 1, 2016
...
class SKCCWASSAward(SKCCAwardBase):
    """SKCC WAS-S Award - Worked All 50 US States (Senator Only)"""

    def __init__(self, database):
        """
        Initialize SKCC WAS-S award

        Args:
            database: Database instance for contact queries
        """
        super().__init__(name="SKCC WAS-S", program_id="SKCC_WAS_S", database=database)
    ...
    def validate(self, contact: Dict[str, Any]) -> bool:
        """
        Check if a contact qualifies for SKCC WAS-S award
        ...
        - Remote station must be Senator (S) member ONLY (NOT Tribune)
        ...
        Returns:
            True if contact qualifies for SKCC WAS-S award
        """
        ...
        if qso_date and qso_date < WASS_EFFECTIVE_DATE:
            logger.debug(
                f"Contact not valid: QSO date {qso_date} before "
                f"WAS-S effective date {WASS_EFFECTIVE_DATE}"
            )
            return False
        ...
        # CRITICAL RULE: Remote station must be Senator (S) ONLY, NOT Tribune
        member_type = get_member_type(skcc_num)
        if member_type != 'S':
            logger.debug(
                f"Contact {callsign} not valid: SKCC number {skcc_num} is not "
                f"Senator (member type: {member_type}). WAS-S requires Senator only."
            )
            return False
    ...
    def calculate_progress(self, contacts: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Calculate SKCC WAS-S award progress

        Tracks which US states have been worked via contacts with Senator
    ...
    def get_requirements(self) -> Dict[str, Any]:
        """
        Return SKCC WAS-S award requirements

        Returns:
            Award requirements dictionary
        """
        return {
            'name': 'SKCC WAS-S Award',
            'description': 'Contact Senator members in all 50 US states',
            'base_requirement': 'Contacts with Senator members in all 50 states',
            'modes': ['CW'],
            'bands': ['All'],
            ...
            'states_required': 50,
            'member_types_required': ['Senator (S) ONLY'],
            'special_rules': [
                'CRITICAL: Remote station must be Senator member ONLY (NOT Tribune)',
                'Contacts may be made on any band (including WARC)',
                'Single-band endorsements available (any individual band)',
                'WAS-S-QRP endorsement available (≤5W power for all contacts)',
                'Mobile/portable Senator operations count regardless of home state',
                'Applicants need not hold Senator status themselves',
    ...
    def get_endorsements(self) -> List[Dict[str, Any]]:
        """Return SKCC WAS-S endorsement levels"""
        return [
            {'level': 'WAS-S', 'states': 50, 'description': 'All 50 US states (Senator only)'},
            {'level': 'WAS-S-160M', 'states': 50, 'description': 'All 50 states on 160M (Senator only)'},
            {'level': 'WAS-S-80M', 'states': 50, 'description': 'All 50 states on 80M (Senator only)'},
            {'level': 'WAS-S-40M', 'states': 50, 'description': 'All 50 states on 40M (Senator only)'},
            {'level': 'WAS-S-20M', 'states': 50, 'description': 'All 50 states on 20M (Senator only)'},
            {'level': 'WAS-S-QRP', 'states': 50, 'description': 'All 50 states at QRP (≤5W, Senator only)'},
        ]
